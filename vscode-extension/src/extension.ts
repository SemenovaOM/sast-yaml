...
function isAnsiblePlaybook(document: vscode.TextDocument): boolean {
...
async function analyzeDocument(document: vscode.TextDocument, 
                              ...
                              silent: boolean = false): Promise<Violation[]> {
    ...
    try {
        ...
        const violations: Violation[] = JSON.parse(result).violations || [];
		...
		vscode.window.showWarningMessage(
			`Найдено ${violations.length} ${problemWord} безопасности (${highCount} ${criticalWord}) в файле ${path.basename(document.fileName)}`,
			'Показать отчёт'
		).then(selection => {
...
function createDiagnostic(violation: Violation, document: vscode.TextDocument): vscode.Diagnostic {
...
function findViolationPosition(violation: Violation, document: vscode.TextDocument): vscode.Position {
...
function showFullReport(violations: Violation[], filename: string) {
    const panel = vscode.window.createWebviewPanel(
        'sastyamlReport',
        `Отчет - ${path.basename(filename)}`,
...
function getReportHtml(violations: Violation[], filename: string): string {
    const highCount = violations.filter(v => v.severity === 'HIGH').length;
